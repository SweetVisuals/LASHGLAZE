        ...
        <div className="group transition-all duration-500">
          <div className="p-3 md:p-5 bg-paper shadow-[inset_0_2px_10px_rgba(0,0,0,0.05)] focus-within:bg-paper focus-within:shadow-[inset_0_2px_20px_rgba(0,0,0,0.08)] transition-all duration-300">
             <PaymentElement options={{ 
                ...
                business: {
                  name: storeSettings.name || 'LashGlaze',
                }
             } as any} />
          </div>
        </div>
...
export const Checkout: React.FC<CheckoutProps> = ({ onBack, onSuccessRedirect }) => {
  ...
  const handleApproveAuth = async (paymentDetails?: { stripe_payment_intent_id?: string, paypal_order_id?: string, payment_method_id?: string, paypal_email?: string }) => {
    setIsProcessing(true);
    ...
    try {
      ...
      if (orderResult) {
        ...
        setIsProcessing(false);
        setIsError(true);
        setErrorMessage('We encountered an issue while archiving your order data in our system. Your payment session has been suspended.');
      }
    } catch (err) {
      ...
      <div className="bg-paper min-h-[80vh] flex flex-col items-center justify-center px-4 py-24 text-center">
        ...
        >
          ...
          <div className="flex flex-col gap-4">
             ...
             <button onClick={onBack} className="text-[10px] uppercase tracking-[0.2em] font-bold text-muted hover:text-ink transition-colors">Return to Shop</button>
          </div>
        </motion.div>
      </div>
    );
        ...
        >
          ...
          <div className="flex items-center justify-center gap-2 mb-12">
            ...
            <p className="text-[9px] uppercase tracking-[0.2em] font-bold text-green-600/80">
              {isCustomerLoggedIn ? 'Securing your profile... tracking order in 1s' : 'Order finalized successfully'}
            </p>
            <div className="h-[2px] w-4 bg-green-600 animate-pulse" />
          </div>
          ...
          <p className="text-sm text-topbarText/70 leading-relaxed mb-12">
            A secure confirmation has been dispatched to your email. You will receive cryptographic tracking details once your order leaves our facility.
          </p>
                      ...
                      {existingActiveSubscription && (
                         <div className="px-8 pb-8 pt-2">
                            <p className="text-[9px] text-emerald-700/60 font-bold uppercase tracking-widest">
                               Note: You may only maintain one active replenishment cycle at a time to ensure exclusive priority remains available for all members.
                            </p>
                         </div>
                      )}
                   </div>
                            ...
                            <div className="max-w-xl w-full text-center">
                              <div className="mb-8">
                                <motion.div 
                                  initial={{ scale: 0.8 }}
                                  animate={{ scale: 1 }}
                                  className="w-24 h-24 bg-ink text-paper mx-auto mb-6 flex items-center justify-center shadow-[0_20px_40px_rgba(0,0,0,0.3)]"
                                >
                                  <ShieldCheck size={40} strokeWidth={1} />
                                </motion.div>
                                <h3 className="font-serif text-4xl italic text-ink mb-2">PayPal Gateway</h3>
                                <p className="text-[10px] uppercase tracking-[0.4em] font-bold text-muted">Secure Automatic Checkout</p>
                              </div>
                              ...
                              {(!customerInfo.firstName || !customerInfo.lastName || !customerInfo.email || !customerInfo.address || !customerInfo.apt || !customerInfo.city || !customerInfo.postalCode) ? (
                                <div className="bg-accent/5 p-8 text-center shadow-inner">
                                  <AlertCircle size={24} className="text-muted mx-auto mb-4 opacity-20" strokeWidth={1} />
                                  <p className="text-[9px] text-muted max-w-xs mx-auto leading-relaxed uppercase font-bold tracking-[0.3em]">
                                    Please provide your full shipping and contact information above to unlock secure PayPal payment.
                                  </p>
                                </div>
                              ) : (!agreeTerms || !agreeData) ? (
                                <div className="bg-accent/5 p-8 text-center shadow-inner">
                                  <AlertCircle size={24} className="text-muted mx-auto mb-4 opacity-20" strokeWidth={1} />
                                  <p className="text-[9px] text-muted max-w-xs mx-auto leading-relaxed uppercase font-bold tracking-[0.3em]">
                                    Please review and check the agreements at the bottom of the page to unlock PayPal checkout.
                                  </p>
                                </div>
                              ) : (
                                <div className="w-full mt-8 max-w-md mx-auto">
                                  <PayPalScriptProvider options={{ 
                                    "client-id": (import.meta as any).env.VITE_PAYPAL_CLIENT_ID || "sb",
                                    ...
                                    <PayPalButtons
                                      ...
                                      onError={(err) => {
                                        ...
                                        setErrorMessage("An error occurred during PayPal checkout. Please try again.");
                                        setIsError(true);
                                      }}
                                    />
                              ...
                              <div className="flex items-center justify-center gap-4 opacity-30 mt-8">
                                ...
                                <span className="text-[8px] font-bold tracking-[0.3em] uppercase">Secured by {storeSettings.name || 'LashGlaze'}</span>
                                <div className="h-[1px] flex-grow bg-ink/20" />
                              </div>
                            </div>
                          </motion.div>
                              ...
                              <div className="space-y-12">
                                ...
                                <div className="flex gap-8 items-start">
                                  ...
                                  <div className="space-y-4 flex-grow">
                                    ...
                                    <div className="bg-paper p-8 shadow-[0_15px_35px_rgba(0,0,0,0.05)] relative overflow-hidden group">
                                      ...
                                      <p className="text-3xl font-serif italic text-ink">{total.toFixed(2)} USDC</p>
                                ...
                                <div className="flex gap-8 items-start">
                                  ...
                                  <div className="space-y-6 flex-grow">
                                    <p className="text-[9px] uppercase tracking-[0.2em] font-bold text-muted">Destination Wallet</p>
                                        ...
                                        try {
                                          const { data, error } = await supabase.functions.invoke('verify-crypto-payment', {
                                            body: { 
                                              ...
                                              destinationAddress: storeSettings.cryptoUsdcAddress 
                                            }
                                          });
                ...
                {isSuccess ? (
                  ...
                  >
                    ...
                    <div className="bg-accent/5 p-8 mb-8 space-y-4 text-left">
                       ...
                       <div className="flex justify-between text-[10px] uppercase tracking-widest font-bold">
                          ...
                          <span className="text-ink">Expedited Shipping</span>
                       </div>
                  ...
                  <>
                    ...
                    <div className="bg-paper shadow-[inset_0_2px_15px_rgba(0,0,0,0.05)] p-6 mb-8">
                      ...
                      <p className="text-xs font-bold text-ink mb-4">{storeSettings.name || 'LashGlaze'}</p>
